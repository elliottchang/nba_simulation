...
class GameEngine:
    def __init__(
        ...
        # game vars
        self.quarter = 1
        self.poss = np.random.choice([1, -1]) # 1 for home, -1 for away, random for jumpball
        self.play = 1 # number of plays in the game
        self.quarter_duration = 12*60 #seconds
        self.ot_duration = 5*60 #seconds
        self.home_score = 0
        self.away_score = 0
        self.home_fouls = 0
        self.away_fouls = 0
        self.deadball = False # True if the ball is not in play (foul, out of bounds, timeout, etc.)
        ...
        # Track minutes played (for stats)
        self.player_minutes = {pid: 0.0 for pid in all_home_players + all_away_players}
        
        # Substitution tracking
        self.substitution_log = []  # Track all subs for game recap
        
        # Timeout tracking (NBA: 7 timeouts per game)
    ...
    def _make_team_substitutions(self, team: str, close_game: bool, blowout: bool):
        ...
        """
        lineup = self.home_lineup if team == 'home' else self.away_lineup
        boxscore = self.hometeam_boxscore if team == 'home' else self.awayteam_boxscore
    ...
    def step(self):
        """
        Simulate one possession and update game state.
        
        This method does the following:
        1. Get possession prediction from the model
        2. Update the appropriate boxscore(s) based on the result
        3. Check if possession changes teams
        4. Handle special cases (offensive rebounds, fouls)
        """
        ...
        # STEP 1: Get lineups for offensive and defensive teams
        if self.poss == 1: # home team has possession
            offensive_lineup = self.home_lineup  # List of player IDs
            defensive_lineup = self.away_lineup
            offensive_boxscore = self.hometeam_boxscore
            defensive_boxscore = self.awayteam_boxscore
        else: # away team has possession
            offensive_lineup = self.away_lineup
            defensive_lineup = self.home_lineup
            offensive_boxscore = self.awayteam_boxscore
            defensive_boxscore = self.hometeam_boxscore
        
        # STEP 2: Get detailed possession prediction with fatigue
        # Calculate fatigue multipliers for all players on court
        fatigue_multipliers = {}
        for player_id in offensive_lineup + defensive_lineup:
            fatigue_multipliers[player_id] = self.get_fatigue_multiplier(player_id)
        
        # Get prediction with fatigue effects
        result = self.predictor.sample_possession_detailed(
            offensive_lineup=offensive_lineup, 
            defensive_lineup=defensive_lineup, 
            season=self.season, 
            score_margin=self.home_score - self.away_score if self.poss == 1 else self.away_score - self.home_score, 
            period=self.quarter,
            fatigue_multipliers=fatigue_multipliers  # Pass fatigue to model
        )
        # 
        # Expected format from predictor.sample_possession_detailed():
        # {
        #     'outcome': '2PT_make',           # What happened
        #     'player_id': 203999,              # Primary player
        #     'points': 2,                      # Points scored
        #     'fga': 1,                         # Field goal attempted (0 or 1)
        #     'fgm': 1,                         # Field goal made (0 or 1)
        #     '3pa': 0,                         # 3-pointer attempted (0 or 1)
        #     '3pm': 0,                         # 3-pointer made (0 or 1)
        #     'fta': 0,                         # Free throw attempted (0 or 1)
        #     'ftm': 0,                         # Free throw made (0 or 1)
        #     'to': 0,                          # Turnover (0 or 1)
        #     'possession_changes': True,       # Does possession switch?
        #     
        #     # Optional secondary stats (if using EnhancedPossessionResult):
        #     'assist_player_id': 201939,       # Who assisted (or None)
        #     'rebound_player_id': 201142,      # Who rebounded (or None)
        #     'rebound_type': 'defensive',      # 'offensive' or 'defensive'
        #     'steal_player_id': 2544,          # Who stole (or None)
        #     'block_player_id': 203507         # Who blocked (or None)
        # }
        
        
        # STEP 3: Update offensive player's boxscore
        # Why: The player who ended the possession gets credit for all primary stats
        # All stats are binary (0 or 1) so we can safely add them
        player_id = result['player_id']
        offensive_boxscore.loc[player_id, 'PTS'] += result['points']
        offensive_boxscore.loc[player_id, 'FGA'] += result['fga']
        offensive_boxscore.loc[player_id, 'FGM'] += result['fgm']
        offensive_boxscore.loc[player_id, '3PA'] += result['3pa']
        offensive_boxscore.loc[player_id, '3PM'] += result['3pm']
        offensive_boxscore.loc[player_id, 'FTA'] += result['fta']
        offensive_boxscore.loc[player_id, 'FTM'] += result['ftm']
        offensive_boxscore.loc[player_id, 'TO'] += result['to']
        
        # Get player name from boxscore (already loaded, no API call needed)
        player_name = offensive_boxscore.loc[player_id, 'PLAYER']
        print(f"{player_name}: {result['outcome']}")
        
        # update plus/minus
        for player_id in offensive_lineup:
            offensive_boxscore.loc[player_id, '+/-'] += result['points']
        for player_id in defensive_lineup:
            defensive_boxscore.loc[player_id, '+/-'] -= result['points']
        
        # STEP 4: Update team score
        # Why: Team score needs to reflect the points just scored
        if self.poss == 1:
            self.home_score += result['points']
            self.home_boxscore = offensive_boxscore
        else:
            self.away_score += result['points']
            self.away_boxscore = offensive_boxscore
        
        # STEP 5: Energy depletion and rest (Option C Energy Pool System)
        # Calculate possession time in minutes
        possession_time = np.random.binomial(n=24, p=0.7) / 60.0  # ~16-17 seconds → minutes
        self.quarter_duration -= possession_time * 60 # subtract possession time from quarter duration

        # Update minutes played for players on court and in boxscore
        for player_id in offensive_lineup + defensive_lineup:
            # Each player on court gets the FULL possession time added
            self.player_minutes[player_id] += possession_time
            
            # Update boxscore with float minutes
            if player_id in offensive_boxscore.index:
                offensive_boxscore.loc[player_id, 'MIN'] += possession_time
            if player_id in defensive_boxscore.index:
                defensive_boxscore.loc[player_id, 'MIN'] += possession_time
        
        # Deplete energy for all players on court
        for player_id in offensive_lineup + defensive_lineup:
            base_cost = 1.0  # Base energy cost per possession
            intensity_bonus = 0.0
            
            # Extra cost for the player who ended the possession
            if player_id == result['player_id']:
                intensity_bonus += 0.5
                
                # Even more cost for high-intensity plays
                if result['outcome'] in ['2PT_make', '2PT_miss']:
                    intensity_bonus += 0.3  # Driving to basket is tiring
                elif result['to'] > 0:
                    intensity_bonus += 0.2  # Turnovers involve scrambling
            
            # Deplete energy
            self.deplete_energy(player_id, base_cost, intensity_bonus)
        
        # Rest players on the bench (regenerate energy)
        all_players = list(self.player_energy.keys())
        on_court = offensive_lineup + defensive_lineup
        ...
        # Hybrid strategy: time-based + fatigue + game situation
        self.check_substitutions()
        

    def simulate_game(self) -> None: # TODO: Add overtime to simulation
    ...
    def __str__(self) -> str:
        return f"Q{self.quarter} - {self.hometeam.team_abbr} vs {self.awayteam.team_abbr}\n      {self.home_score}  -  {self.away_score}"
